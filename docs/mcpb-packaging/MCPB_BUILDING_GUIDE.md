...
4. **UI/UX**: Follows Podman Design System
...
### Tool Integration MCP (Blender, Podman, Git)
...
```json
{
  ...
  "server": {
    "type": "python", 
    "entry_point": "src/podman_mcp/server.py",
    "mcp_config": {
      "command": "python",
      "args": ["-m", "podman_mcp.server"],
      "cwd": "src",
      "env": {
        "PYTHONPATH": "src",
        "PODMAN_EXECUTABLE": "${user_config.podman_path}"
      }
    }
  }
}
