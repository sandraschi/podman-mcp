...
_max_entries = max(
    100,
    min(int(os.environ.get("PODMAN_MCP_LOG_MAX_ENTRIES", str(_DEFAULT_MAX))), 50_000),
)
_lock = Lock()
...
def export_logs(
    *,
    ...
    if format == "csv":
        ...
        return buffer.getvalue(), "text/csv", f"podman-mcp-logs-{stamp}.csv"

    body = json.dumps({"exported_at": datetime.now(UTC).isoformat(), "entries": entries}, indent=2)
    return body, "application/json", f"podman-mcp-logs-{stamp}.json"
