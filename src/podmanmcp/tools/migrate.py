        ...
        if operation == "docker_compose_to_podman":
            ...
            if "version:" in converted:
                converted = converted.replace(
                    'version: "3"', '# version: "3"  # podman ignores version; kept for reference'
                )
            ...
            return {
                ...
                "data": {
                    "output": dest,
                    "changes": changes,
                    "warnings": [c for c in changes if "may need" in c.lower()],
                },
            ...
            scan_dirs = (
                [source_path] if source_path and os.path.isdir(source_path) else [os.path.expanduser("~"), os.getcwd()]
            )
            ...
            if not pull_res["success"]:
                return _error_response(
                    f"Failed to pull image '{image_name}': {pull_res.get('stderr')}", "migrate_pull_failed"
                )
            tag = new_name or image_name.replace("docker.io/", "").replace("library/", "")
            if tag != image_name:
                await run_podman_command(["tag", image_name, tag])
            return {
                "success": True,
            ...
            docker_only = [
                (
                    "deploy:",
                    "deploy configuration (Swarm mode) is not supported by podman-compose; use 'scale' or pod-level replication",
                ),
            ...
            for pattern, explanation in docker_only:
                if pattern.lower() in content.lower():
                    issues.append(
                        {
                            "pattern": pattern,
                            "severity": "warning" if "may" in explanation else "error",
                ...
                issues.append(
                    {
                        "pattern": "version:",
                        "severity": "info",
                        "explanation": "version field is ignored by podman-compose (safe to keep)",
                    }
                )
            if "depends_on:" in content and "condition: service_healthy" in content:
                issues.append(
                    {
                        "pattern": "depends_on condition",
                        "severity": "warning",
                        "explanation": "service_healthy condition may not work; podman-compose polling is simpler",
                    }
                )
            ...
            if os.path.isfile(source_path) and source_path.lower().endswith("dockerfile"):
                ...
                return {
                    "success": True,
                    "message": f"Dockerfile copied to Containerfile at {dest}.",
                    "data": {"from": dockerfile, "to": dest},
                }
            else:
                return _error_response(
                    "source_path must be a Dockerfile or a directory containing one", "validation_failed"
                )
            ...
            return {
                ...
                "data": {
                    ...
                    "docker_load_cmd": f"docker load -i {dest}",
                },
            }
